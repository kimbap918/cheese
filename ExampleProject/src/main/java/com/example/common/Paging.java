...
public class Paging {
    ...
    private void makePaging() {
        ...
        //prevPageNo
        //현재 페이지가 첫번째 페이지 이거나 (현재페이지-5)가 첫번째 페에지 보다 작을 경우 이전 페이지는 첫번째 페이지
        if (isNowFirst || firstPageNo > (startPage-1)) {
            this.setPrevPageNo(firstPageNo);
        //아니면
        } else{
        	this.setPrevPageNo(startPage-1);
        }
        ...
        //nextPageNo
        //현재 페이지가 마지막 페이지 이거나 (현재페이지+5)가 마지막 페이지보다 클 경우 다음페이지는 마지막 페이지
        if (isNowFinal || finalPage < (endPage+1)) {
            this.setNextPageNo(finalPage);
        //아니면
        } else {
    		this.setNextPageNo(endPage+1);
        }
